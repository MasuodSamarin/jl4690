...
void enter_menulist_display(u32 arg)
{
    ...
    //注册菜单按键表
    menu_key_msg_register(UI_TASK_NAME,(u8*)&menu_adkey_msg_table);
...
//    menu_list_pt->bs_total_dir = 18;
    ...
    menu_list_pt->item_total = file_bs_open_handle(menu_browser_mapi) + 1;//包含返回菜单
...
void start_main_menu(u32 arg)
{
    ...
    menu_list_pt->item_total = sizeof(main_app_img_group)/(2) -1;//取消设置应用
