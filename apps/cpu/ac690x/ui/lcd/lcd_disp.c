...
void  lcd_check_sys_vol(void)//静音显示
{
#if 1
    if(*(UI_var.sys_vol) == 0)
    {
        ...
        if(rec_var_p)
        {
            ...
            if(rec_ctl_var)
            {
                ...
                if(rec_ctl_var->enable != ENC_STOP)
                {
                    ...
                    printf("rec_time = %d\n",rec_time);
                    itoa2_api(rec_time/60,(u8 *)&lcd_time_str[8]);
...
//    puts("lcd_alarm_set start\n");
    ...
    if(rtc_var)
    {
        ...
        {
            _lcd_area_size_t *area_pt = &time_set_backgroup[rtc_var->alarm_set.coordinate];
