...
static void modify_eq_new_value(u8 eq_sn,s8 *eq_value)
{
    ...
    {
        user_eq_seg_gain[eq_sn][cnt] = eq_value[cnt];
    }
}
...
static void reset_eq_new_value(u8 eq_sn)
{
    ...
    memcpy(user_eq_seg_gain[eq_sn],user_eq_seg_gain_default[eq_sn],10);
...
static void save_eq_table(void)
{
    ...
    rcsp_put_buf((void*)user_eq_seg_gain,60);
...
u32 cbk_send_apic_data(void)
{
    ...
    {
        rcsp_db_puts("------set_pasue\n");
        os_taskq_post(RDEF_MUSIC_TASK_NAME, 1, MSG_MUSIC_PAUSE);
        play_flag = 1;
        os_time_dly(5);
    }

    send_cnt = rcsp_cbk_answer_apic_send();
    ...
    for(cnt = 0; cnt < len; cnt += 4)
    {
        ...
        case JL_FRAME_ID_EQCU:
            ...
            else
            {
                ...
                {
                    data_buf_addr[data_cnt++] = 1 << get_bt_eq_mode();
                }
                else
                {
                    data_buf_addr[data_cnt++] = 1 << eq_mode;
                }
            }
            frame_p->value_len = 1;
            break;
...
u32 rcsp_bt_callback(rcsp_cbk_msg_e pkg_type,u8 *data,u16 len)
{
    ...
    case RCSP_CBK_MSG_COMM_EQ:
        ...
        if(byte_val < 6)
        {
            msg = MSG_BT_MUSIC_EQ;
            rcsp_db_puts("sel eq\n");
            param[param_cnt] = (int)byte_val | 0x80;
            param_cnt++;
            rcsp_eq_sw_flag = 1;
        }
        else
...
//u32 rcsp_apptask_callback(rcsp_cbk_msg_e pkg_type,u8 *data,u16 len)
//{
//    switch(pkg_type)
//    {
//    case RCSP_CBK_MSG_CHANGE_MODE:
...
//
//    case RSCP_CBK_MSG_REQUEST_FRAME:
//        cbk_request_frame_deal(data,len);
//        break;
//
//        //private msg
//    default:
