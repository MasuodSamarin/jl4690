...
{
    return vm_read(vm_ptr[index],data_buf);
}
/*----------------------------------------------------------------------------*/
...
vm_err vm_init_api(struct flash_cfg *cfg)
{
    ...
    if(vm_init(cfg->flash_size - (68L*1024), cfg->flash_base, vm_mode, 1)!= VM_ERR_NONE)
    {
        puts("\n\n\n\n----WARN-vm_init_err----\n");
        return VM_ERR_INIT;
    }
