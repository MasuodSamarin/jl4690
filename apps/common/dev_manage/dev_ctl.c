...
static void dev_reg_ctrl(void)
{
    ...
    sd0_api_set.controller_io = SD0_IO_B;  //SD0_IO_A;        //SD0_IO_A：SD0控制器B出口，SD0_IO_B：SD0控制器B出口

#ifdef SUPPORT_SDMMC0_EN_DET_USE_CMD
...
#if RCSP_LIGHT_HW||defined(user_sd_1_a)
...
u32 dev_detect_fun(u32 info)
{
    ...
    phone_check_api(info); //耳机检测
#endif

#if MIC_CHECK_ENABLE
    MIC_check_api(info); //MIC检测
#endif
